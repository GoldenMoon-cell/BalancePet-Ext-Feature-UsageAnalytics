...
汇总本地记录的请求元数据：Token 数量、缓存读取、请求成功率、首 Token 时间、输出速度、
提供方、账户和模型。
...
同时保留手动刷新按钮。只有客户端主动上报 Token、缓存和首 Token 字段时，对应卡片才会
显示数值；否则会显示“未上报”。

The dashboard refreshes when event files change and at least once per minute as a fallback.
The header shows a live countdown and provides manual refresh. Token, cache, and TTFT cards
are populated only when the client reports those counters; otherwise they remain “未上报 / Not reported”.
...
the core version; the current package version is `0.2.10`.
